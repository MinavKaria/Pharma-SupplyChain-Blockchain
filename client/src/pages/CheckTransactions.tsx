...
const SupplyChainVisualizer = () => {
  ...
  const [dimensions, setDimensions] = useState<Dimensions>({ width: 1000, height: 700 });
  ...
  useEffect(() => {
    const batchesArray = allBatchesData as unknown as bigint[][];
    if (batchesArray && batchesArray.length > 0) {
      const ids = batchesArray[0]; 
  ...
  if (batchDetails && selectedBatch !== null) {
    ...
    const typedBatchDetails = batchDetails as unknown as BatchDetailsResponse;
    const transfers = typedBatchDetails[4];
  ...
  const renderGraph = () => {
    ...
    const buildHierarchy = (transfers: Transfer[]): HierarchyResult | null => {
      ...
      if (!firstTransfer && transfers.length > 0) {
        ...
        console.log("No manufacturer found in transfers. Using first transfer as root.");
      ...
      const addChildren = (node: HierarchyNode, transfers: Transfer[]): void => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8">
        ...
        <p className="text-gray-600">Blockchain-based visualization of product movement through the supply chain</p>
      </header>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div className="tree-container" style={{ width: '100%', height: '700px', position: 'relative' }}>
          {loading || isLoadingDetails ? (
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-500">No transfer data available for this batch.</p>
            </div>
          ) : (
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        {showTable && (
          <div className="overflow-x-auto">
            {loading || isLoadingDetails ? (
              ...
              <p className="text-gray-500 py-4">No transfer data available for this batch.</p>
            ) : (
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">From</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">From Role</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">To</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">To Role</th>
                    ...
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${transfer.fromRole === 'Manufacturer' ? 'bg-red-100 text-red-800' : 
                          transfer.fromRole === 'Distributor' ? 'bg-green-100 text-green-800' :
                          transfer.fromRole === 'Retailer' ? 'bg-blue-100 text-blue-800' :
                          transfer.fromRole === 'Customer' ? 'bg-yellow-100 text-yellow-800' : 
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${transfer.toRole === 'Manufacturer' ? 'bg-red-100 text-red-800' : 
                          transfer.toRole === 'Distributor' ? 'bg-green-100 text-green-800' :
                          transfer.toRole === 'Retailer' ? 'bg-blue-100 text-blue-800' :
                          transfer.toRole === 'Customer' ? 'bg-yellow-100 text-yellow-800' : 
                          'bg-gray-100 text-gray-800'}`}>
                          {transfer.toRole}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          ...
                          onClick={() => {
                            navigator.clipboard.writeText(`From: ${transfer.from}\nTo: ${transfer.to}\nQuantity: ${transfer.quantity}`);
        ...
        {showTable && transferData.length > 0 && (
          <div className="mt-4 text-sm text-gray-500">
            ...
            <p>* Addresses are truncated for display purposes. Hover to see the full address.</p>
          </div>
        )}
      </div>
    </div>
