...
const SupplyChainVisualizer = () => {
  ...
  const renderGraph = () => {
    ...
    Object.values(edgeGroups).forEach((groupedEdges, index) => {
      ...
      // Use an index-based staggering technique to prevent overlap
      const offset = (index % 5) * 10; // Stagger by multiples of 10px
      const baseAngle = Math.atan2(
        targetPos.x - sourcePos.x,
        targetPos.y - sourcePos.y
      );

      // Use the link length to determine how far out to place the label
      const linkLength = Math.sqrt(
        Math.pow(targetPos.x - sourcePos.x, 2) +
          Math.pow(targetPos.y - sourcePos.y, 2)
      );

      // Place labels at different positions along the curve
